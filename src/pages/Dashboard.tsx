...
const Dashboard = () => {
  ...
  // Fetch dashboard data saat user siap
  useEffect(() => {
    if (!user?.id) return;
    fetchMyDashboard({ recent: 5, months: 6 });
  }, [user?.id, fetchMyDashboard]);

  const stats = useMemo(() => {
    return {
      totalOrders: dashboardStats?.totalOrders ?? 0,
      pendingOrders: dashboardStats?.pendingOrders ?? 0,
      successOrders: dashboardStats?.successOrders ?? 0,
      totalSpent: dashboardStats?.totalSpent ?? 0,
    };
  }, [dashboardStats]);

  const recentTransactions = useMemo(() => {
    // Store sudah mengirim 5 terakhir; tetap amankan bila lebih
  ...
  // Chart data dari backend; fallback ke mock jika kosong
  const chartData = useMemo(() => {
    if (monthlySpending && monthlySpending.length > 0) return monthlySpending;
    ...
    return [
      ...
      {
        name: 'May',
        total: stats.totalSpent > 1000000 ? stats.totalSpent - 600000 : 350000,
      },
      { name: 'Jun', total: stats.totalSpent },
    ];
  }, [monthlySpending, stats.totalSpent]);
        ...
        <main className="flex-1 ai-grid-bg p-6 sm:p-10 lg:p-16">
          <div className="mx-auto w-full max-w-7xl">
            <div className="ai-shell p-8 sm:p-12 lg:p-16 mb-12">
              ...
              <div className="mb-10 animate-fade-in flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <h1 className="font-display text-3xl sm:text-4xl font-bold text-foreground mb-2 leading-tight">
                    Selamat Datang, {user?.name?.split(' ')[0] ?? 'Customer'}!
                  </h1>
                  <p className="text-muted-foreground">
                    Lihat perkembangan belanja dan kelola pesanan Anda.
                  </p>
                  {/* opsional: indikator kecil info */}
              ...
              {/* Error banner (jika fetch dashboard gagal) */}
              {dashboardError ? (
                <div className="mb-8 rounded-2xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
                  {dashboardError}
                </div>
              ) : null}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="font-display text-3xl font-bold">
                          {isLoadingDashboard ? '…' : stats.totalOrders}
                        </p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="font-display text-3xl font-bold">
                          {isLoadingDashboard ? '…' : stats.pendingOrders}
                        </p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="font-display text-3xl font-bold">
                          {isLoadingDashboard ? '…' : stats.successOrders}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="min-w-0">
                        ...
                        <p
                          ...
                          title={formatPrice(stats.totalSpent)}
                        >
                          {isLoadingDashboard ? '…' : formatPrice(stats.totalSpent)}
                        </p>
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                  {/* Spending Chart */}
                  <Card
                    className="glass-card animate-fade-in rounded-3xl"
                          ...
                          {recentTransactions.map((transaction) => (
                            ...
                            >
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <p className="font-medium">#{String(transaction.id)}</p>
                ...
                <div className="space-y-6">
                  <Card
                    className="glass-card animate-fade-in rounded-3xl bg-gradient-to-br from-primary/90 to-primary text-primary-foreground border-none"
                  ...
                  >
                    <CardContent className="p-8">
                      ...
                      <Button
                        variant="secondary"
                        className="w-full text-primary rounded-xl font-bold"
                      >
                        Upgrade Sekarang
                      </Button>
                    </CardContent>
